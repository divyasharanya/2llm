...
function LoginErrorBanner() {
    ...
    return (
      <div className="bg-amber-500/10 border border-amber-500/25 rounded-xl p-4 flex flex-col gap-2">
        <p className="text-amber-400 text-xs font-bold flex items-center gap-2">
          <span>🚫</span> No account found with this email.
        </p>
  ...
  if (error === "AccountExists") {
    return (
      <div className="bg-purple-500/10 border border-purple-500/25 rounded-xl p-4 flex flex-col gap-2">
        <p className="text-purple-400 text-xs font-bold flex items-center gap-2">
          <span>✅</span> You already have an account.
        </p>
...
export default function LoginPage() {
  ...
  const handleGoogleLogin = () => {
    // Tag this OAuth flow as a LOGIN attempt so auth.ts can enforce
    // that the account must already exist in the Users table.
    document.cookie = "auth-intent=login; path=/; max-age=300; SameSite=Lax";
  ...
  return (
    <div className="flex-1 flex flex-col items-center justify-center p-6 bg-[#0D0D0D] min-h-screen font-sans">
      ...
      <div className="relative z-10 w-full max-w-md bg-[#141414] border border-purple-500/10 hover:border-purple-500/20 transition-all rounded-2xl p-8 shadow-[0_0_60px_rgba(124,58,237,0.08)] flex flex-col gap-6">
        ...
        {/* URL-based error banners (NoAccount / AccountExists) */}
        ...
        {activeTab === "google" ? (
          <div className="flex flex-col gap-4">
            ...
            <p className="text-gray-600 text-[10px] text-center">
              For existing accounts only — must have signed up first.
            </p>
          </div>
        ) : (
